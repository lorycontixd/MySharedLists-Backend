<?php
    ...
    $stmt = sqlsrv_query( $conn, "update listinvitations set wasviewed = ? where id = ?" , array($newstate, $invitationid), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
    if ($stmt === false) {
        $errorMsg = sqlsrv_errors()[0]['message'];
        $errorCode = sqlsrv_errors()[0]['code'];
        die("Error: " . $errorCode . " - " . $errorMsg);
    ...
    $inv = new ListInvitation(
        $invitation['id'],
        $invitation['creatorid'],
        $invitation['invitedid'],
        $invitation['listid'],
        $newstate,
        $invitation['dayduration'],
        $invitation['creationdate']
    );
    sqlsrv_free_stmt($stmt);
    sqlsrv_close($conn);
    echo(print_r($inv->newJsonSerialize(), true));
?>